...
if (
  ...
  // Se il gruppo è sospeso, blocca l'esecuzione
  if (groupData.isSuspended) {
    if (now < groupData.suspendedUntil) return;
    // Timeout scaduto, riattiva
  ...
  // Reset contatore se è passato più di 1 minuto
  ...
  // Se superato il limite, attiva sospensione
  if (groupData.count > 2) {
    groupData.isSuspended = true;
    groupData.suspendedUntil = now + 45000; // 45 secondi di blocco
